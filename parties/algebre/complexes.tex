...
\begin{methode}
	...
	fixe le signe de $xy$. Il y a deux racines opposées.
\end{methode}
...
\begin{methode}
	...
	\begin{enumerate}
		...
		\item Factoriser $P(z) = (z - z_0)\,Q(z)$ par identification.
...
Soit $z = \ii y$ ($y \in \R$) une racine. En reportant :
$-\ii y^3 + (4 + \ii)y^2 + (5 + 4\ii)\ii y - 5\ii = 0$, soit après regroupement
$$
\bigl(4y^2 - 4y\bigr) + \ii\bigl(-y^3 + y^2 + 5y - 5\bigr) = 0.
$$
La partie réelle donne $4y(y - 1) = 0$ ; $y = 1$ annule aussi la partie
imaginaire. Donc $z_0 = \ii$ est racine.

Factorisation : $z^3 - (4+\ii)z^2 + (5+4\ii)z - 5\ii = (z - \ii)(z^2 - 4z + 5)$.

$z^2 - 4z + 5 = 0$ : $\Delta = -4$, $z = 2 \pm \ii$.

\textbf{Solutions :} $\{\,\ii\,;\,2 + \ii\,;\,2 - \ii\,\}$.
...
\textbf{Interprétation.} $\arg(Z) = \arg(z - 2) - \arg(z - \ii)
\equiv \bigl(\vect{AM},\,\vect{BM}\bigr)\ [2\pi]$ n'est défini que si
...
\begin{enumerate}
	\item $\abs{z - 1 + \ii} = 3$
	\item $\abs{z - 2} = \abs{z + \ii}$
	\item $\mathrm{Re}(z) = -1$
	\item $\arg(z - \ii) \equiv \dfrac{\pi}{4}\ [2\pi]$
	\item $\dfrac{z - 1}{z + 1} \in \ii\R$
\end{enumerate}
...
\begin{enumerate}
	\item $\abs{z - 1 + \ii} = 2$
	\item $\abs{z - 1 + \ii} = \abs{z - 3 - \ii}$
	\item $\abs{z - 1 + \ii} = 2\,\abs{z - 3 - \ii}$
	\item $\abs{2z - 4} = \abs{z - 3 - \ii}$
\end{enumerate}
...
\textbf{Partie A}
\begin{enumerate}
	\item Calculer $P(2)$. \hfill (0,25 pt)
	\item Déterminer les réels $a$, $b$ tels que
	$P(z) = (z - 2)(z^2 + az + b)$. \hfill (0,5 pt)
	\item Résoudre dans $\C$ l'équation $P(z) = 0$. \hfill (0,75 pt)
\end{enumerate}
...
\textbf{Partie A}
\begin{enumerate}
	\item Interpréter géométriquement $\abs{Z}$ et $\arg(Z)$. \hfill (0,5 pt)
	\item Déterminer et construire l'ensemble $(\Gamma_1)$ des points $M$ tels
	que $\abs{Z} = 1$. \hfill (0,75 pt)
	\item Déterminer et construire l'ensemble $(\Gamma_2)$ des points $M$ tels
	que $Z$ soit un réel strictement négatif. \hfill (0,75 pt)
	\item Déterminer l'ensemble $(\Gamma_3)$ des points $M$ tels que $Z$ soit
	un imaginaire pur. \hfill (0,5 pt)
\end{enumerate}

\textbf{Partie B}
\begin{enumerate}
	\item On pose $z = x + \ii y$. Écrire $\mathrm{Re}(Z)$ et $\mathrm{Im}(Z)$
	en fonction de $x$ et $y$. \hfill (0,75 pt)
	\item Retrouver la nature de $(\Gamma_1)$ et $(\Gamma_3)$ par le calcul. \hfill (0,5 pt)
\end{enumerate}
...
\textbf{Partie A}
\begin{enumerate}
	\item Déterminer les points invariants de $f$. \hfill (0,5 pt)
	\item Montrer que $z'$ est réel si et seulement si $M$ appartient à une
	droite à préciser. \hfill (0,75 pt)
	\item Montrer que $z'$ est imaginaire pur si et seulement si $M$ appartient
	à un cercle privé de deux points, à préciser. \hfill (0,75 pt)
\end{enumerate}

\textbf{Partie B}
\begin{enumerate}
	\item Déterminer l'ensemble des points $M$ tels que $\abs{z'} = 1$. \hfill (0,75 pt)
	\item Déterminer l'ensemble des points $M$ tels que $\abs{z'} = 2$. \hfill (0,75 pt)
\end{enumerate}
...
\textbf{Partie A}
\begin{enumerate}
	\item Vérifier que $(E)$ s'écrit $(z^2 - 2z + 4)^2 = 0$. \hfill (0,5 pt)
	\item En déduire les solutions de $(E)$ et les mettre sous forme
	exponentielle. \hfill (0,75 pt)
\end{enumerate}
...
\begin{enumerate}
	...
	$\bigl(\vect{OA},\vect{OB}\bigr)$. En déduire la nature de $OAB$. \hfill (0,75 pt)
	\item Déterminer l'affixe du point $C$ tel que $OACB$ soit un losange.
	Ce losange est-il un carré ? Justifier. \hfill (0,75 pt)
	\item Déterminer et construire l'ensemble des points $M$ d'affixe $z$ tels
	que $\abs{z - z_A} = \abs{z - z_B}$. \hfill (0,5 pt)
\end{enumerate}
...
\begin{enumerate}
	...
	Déterminer l'écriture complexe de $r$, puis l'affixe de $B' = r(B)$. \hfill (0,75 pt)
	\item Montrer que le triangle $ABB'$ est rectangle isocèle en $A$. \hfill (0,5 pt)
\end{enumerate}

\textbf{Partie B}
\begin{enumerate}
	\item Déterminer et construire l'ensemble $(\mathscr{C})$ des points $M$
	d'affixe $z$ tels que $\abs{z - 1 - \ii} = \abs{z + 1 - 3\ii}$. \hfill (0,75 pt)
	\item Déterminer et construire l'ensemble $(\mathscr{D})$ des points $M$
	tels que $\dfrac{z - z_A}{z - z_B}$ soit un imaginaire pur. \hfill (0,75 pt)
	\item Préciser l'intersection de $(\mathscr{C})$ et $(\mathscr{D})$. \hfill (0,5 pt)
\end{enumerate}
