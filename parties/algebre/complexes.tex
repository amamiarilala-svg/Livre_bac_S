...
\begin{enumerate}
	...
	forme exponentielle. \hfill (0,5 pt)
	\item En déduire la forme algébrique de $z_0^{\,6}$. \hfill (0,5 pt)
	\item Déterminer le plus petit entier $n \geq 1$ tel que $z_0^{\,n}$ soit
	un réel strictement positif. \hfill (0,5 pt)
	\item On note $A_k$ le point d'affixe $z_0^{\,k}$ pour $k \in \{0;1;2\}$.
	Placer $A_0$, $A_1$, $A_2$ et justifier que
	$\arg\!\left(\dfrac{z_0^{\,2}}{z_0}\right) \equiv \dfrac{\pi}{3}\ [2\pi]$. \hfill (0,5 pt)
\end{enumerate}

\textbf{Partie B} — \textit{formule de Moivre}

\begin{enumerate}
	\item Énoncer la formule de Moivre. \hfill (0,25 pt)
	\item En développant $(\cos\theta + \ii\sin\theta)^4$ par le binôme de
	Newton, montrer que
	$\cos(4\theta) = 8\cos^4\theta - 8\cos^2\theta + 1$. \hfill (1 pt)
	\item En déduire les solutions dans $[0\,;\,\pi]$ de l'équation
	$8\cos^4\theta - 8\cos^2\theta + 1 = 0$. \hfill (0,75 pt)
\end{enumerate}
...
\begin{enumerate}
	...
	$\sin\theta$. \hfill (0,25 pt)
	\item Linéariser $\sin^3\theta$, c'est-à-dire l'écrire sous la forme
	$a\sin\theta + b\sin(3\theta)$ avec $a$, $b$ réels à préciser. \hfill (1 pt)
	\item En déduire la valeur exacte de
	$\displaystyle I = \int_0^{\pi}\sin^3\theta\dd\theta$. \hfill (0,5 pt)
	\item Vérifier la cohérence du signe de $I$ à l'aide du signe de
	$\sin\theta$ sur $[0\,;\,\pi]$. \hfill (0,25 pt)
\end{enumerate}
