...
\begin{vraievie}{Le chiffre qui protège votre compte bancaire}
	Un relevé d'identité bancaire (RIB) comporte un numéro de compte suivi
	d'une \textbf{clé de contrôle} à deux chiffres. Cette clé n'est pas
	choisie au hasard : elle se calcule à partir du reste de la division du
	numéro de compte par $97$ --- exactement une congruence.

	Si $N$ désigne le numéro de compte, la clé $k$ (comprise entre $0$ et
	$96$) vérifie
	\[
	k \equiv 97 - N\ [97], \qquad 0\leqslant k\leqslant 96.
	\]
	Autrement dit, $N+k \equiv 0\ [97]$.

	\textbf{Exemple.} Pour $N = 123\,456\,789$ :
	\[
	123\,456\,789 = 1\,272\,750 \times 97 + 39,
	\]
	donc $N \equiv 39\ [97]$, et la clé vaut $k = 97-39 = 58$.

	Ce principe protège contre les erreurs de saisie : si un seul chiffre de
	$N$ est mal recopié, la nouvelle valeur $N'$ vérifie presque toujours
	$N' \not\equiv N\ [97]$, donc la clé recalculée ne correspond plus --- et
	l'erreur est détectée avant même de connaître le compte réel.

	\reserve{Algorithme simplifié pour illustrer le principe. La norme
	bancaire réelle (RIB français, IBAN --- norme ISO~7064) ajoute une
	conversion des lettres en chiffres, mais repose sur la même idée de
	congruence modulo $97$.}
\end{vraievie}
...
\begin{listecol}(2)
	\task $\bar{2}x + \bar{3} = \bar{1}$
	\task $x^2 + \bar{2}x = \bar{3}$
	\task $x^2 + \bar{4}x + \bar{4} = \bar{0}$
\end{listecol}
...
\begin{enumerate}
	...
	\item Résoudre dans $\Z/5\Z$ l'équation $x^2 + \bar{2}\,x = \bar{3}$.
...
\begin{enumerate}
	...
	\end{cases}
	$$
	\item Effectuer la division euclidienne de $-47$ par $6$.
...
\begin{enumerate}
	...
	\item
	\begin{enumerate}
		\item Donner une solution particulière de l'équation $19x - 8y = 1$.
...
\begin{enumerate}
	...
	$3a = 2b$.
	\item Déterminer les chiffres $a$ et $b$, puis la valeur de $N$.
	\item Écrire $N$ en base $2$ et en base $16$.
	\item Quel est le reste de la division euclidienne de $N^{2026}$ par $9$ ?
\end{enumerate}
...
\begin{enumerate}
	\item Vérifier que $5\,a_n - 2\,b_n = 1$.
	\item En déduire que $a_n$ et $b_n$ sont premiers entre eux pour tout $n$.
	\item
	\begin{enumerate}
		\item Montrer que $\mathrm{PGCD}(2n + 3\,;\,n + 1) = 1$.
		\item Montrer que $n^2 + 1 = (n + 1)(n - 1) + 2$, puis que
		$\mathrm{PGCD}(n^2 + 1\,;\,n + 1)$ vaut $1$ ou $2$ selon la parité de $n$.
	\end{enumerate}
	\item Résoudre dans $\Z^2$ l'équation $a_n\,x + b_n\,y = 4$ pour $n = 1$.
\end{enumerate}
