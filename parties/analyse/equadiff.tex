...
\begin{tikzpicture}[>=Stealth, scale=0.95]
	\begin{scope}
		...
		\draw[thick,domain=-1.0:2.7,samples=100] plot (\x,{exp(-\x)});
		\draw[thick,domain=-0.35:2.7,samples=100] plot (\x,{2*exp(-\x)});
		\draw[thick,domain=-2.7:1.0,samples=100] plot (\x,{-exp(-\x)});
	\end{scope}
	\draw[->] (-2.9,0) -- (3.1,0) node[right,font=\small] {$x$};
	\draw[->] (0,-2.8) -- (0,2.9) node[above,font=\small] {$y$};
	\node[font=\scriptsize] at (2.5,0.55) {$C=1$};
	\node[font=\scriptsize] at (1.55,1.35) {$C=2$};
	\node[font=\scriptsize] at (1.55,-0.75) {$C=-1$};
	\node[font=\scriptsize,gray] at (-1.9,0.3) {$C=0$};
\end{tikzpicture}
\end{center}

\begin{enumerate}
	\item Donner la forme générale des solutions de $(E)$.
	\item Identifier la solution qui vérifie $y(0)=2$.
...
\begin{enumerate}
	\setcounter{enumi}{4}
	\item Donner la température initiale. \hfill \mbox{(0,25~pt)}
	\item Déterminer $\displaystyle\lim_{t\to+\infty}T(t)$ et l'interpréter. \hfill \mbox{(0,5~pt)}
	\item Calculer $T'(t)$ et étudier son signe sur $[0\,;+\infty[$. \hfill \mbox{(1~pt)}
	\item À quel instant $T$ est-elle maximale ? Donner cette température à
	$0{,}1\,^{\circ}$C près. \hfill \mbox{(0,5~pt)}
\end{enumerate}
...
\textbf{Partie A}
\begin{enumerate}
	\item Calculer $g'(x)$ et vérifier que $g$ est solution de $(E)$. \hfill \mbox{(1~pt)}
	\item Résoudre l'équation homogène $(E_0):y'-2y=0$. \hfill \mbox{(0,5~pt)}
	\item Montrer que $f$ est solution de $(E)$ si et seulement si $f-g$ est
	solution de $(E_0)$. \hfill \mbox{(0,75~pt)}
	\item En déduire la solution générale de $(E)$. \hfill \mbox{(0,5~pt)}
\end{enumerate}
...
\begin{enumerate}
	...
	$\displaystyle\lim_{x\to+\infty}g(x)$ (poser $X=\e^{-2x}$). \hfill \mbox{(1~pt)}
	\item En déduire une asymptote à $\mathcal C_g$. \hfill \mbox{(0,5~pt)}
\end{enumerate}

\exercice{Sujet type Bac — tangente imposée \hfill \difficile}
On considère $(E):y''-3y'+2y=0$.
\begin{enumerate}
	\item Résoudre $(E)$ sur $\R$. \hfill \mbox{(0,75~pt)}
	\item Déterminer la solution $f$ dont la courbe passe par $A(0\,;4)$ et
	admet en $A$ une tangente parallèle à l'axe des abscisses. \hfill \mbox{(1~pt)}
	\item Étudier les variations de $f$ sur $\R$ et déterminer
	$\displaystyle\lim_{x\to-\infty}f(x)$, $\displaystyle\lim_{x\to+\infty}f(x)$. \hfill \mbox{(1~pt)}
	\item Étudier la convexité de $f$ ; montrer que $\mathcal C_f$ a un point
	d'inflexion. \hfill \mbox{(0,75~pt)}
\end{enumerate}
...
\begin{enumerate}
	\item Donner la solution générale de $(E)$. \hfill \mbox{(0,5~pt)}
	\item À $t=0$, le pendule est écarté de $\theta_0$ et lâché sans vitesse.
	Déterminer $\theta(t)$. \hfill \mbox{(0,75~pt)}
	\item Montrer que l'accélération $\theta''(t)$ est proportionnelle à
	$\theta(t)$ ; préciser le coefficient. \hfill \mbox{(0,5~pt)}
	\item On pose $E(t)=\bigl(\theta'(t)\bigr)^{2}+\omega^{2}\bigl(\theta(t)\bigr)^{2}$.
	Montrer que $E$ est \textbf{constante}. \hfill \mbox{(1~pt)}
\end{enumerate}

\exercice{Sujet type Bac — équation avec second membre trigonométrique \hfill \difficile}
On considère $(E):y''+y=\cos x$.
\begin{enumerate}
	\item Résoudre l'équation homogène $(E_0):y''+y=0$. \hfill \mbox{(0,5~pt)}
	\item Montrer que $u(x)=\dfrac12\,x\sin x$ est une solution particulière
	de $(E)$. \hfill \mbox{(1~pt)}
	\item En déduire la solution générale de $(E)$. \hfill \mbox{(0,5~pt)}
	\item Déterminer la solution $f$ telle que $f(0)=0$ et $f'(0)=0$. \hfill \mbox{(0,75~pt)}
	\item Calculer $\displaystyle\int_0^{\pi}f(x)\dd x$. \hfill \mbox{(0,75~pt)}
\end{enumerate}
...
\begin{enumerate}
	...
	$y''(\e^{t})$ et $\e^{t}$. \hfill \mbox{(1~pt)}
	\item Montrer que $y$ est solution de $(E)$ sur $I$ si et seulement si $z$
	est solution d'une équation $(E'):z''+2z'+z=0$. \hfill \mbox{(1~pt)}
	\item Résoudre $(E')$ sur $\R$. \hfill \mbox{(0,5~pt)}
	\item En déduire les solutions de $(E)$ sur $I$. \hfill \mbox{(0,75~pt)}
\end{enumerate}
